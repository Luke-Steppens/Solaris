...
def send_angles(x, y):
    ...
    # Flip x for mirror effect so that left/right movement is proper
    flipped_x = WIDTH - x

    # Map only the x coordinate to the base servo
    angle_base = int(map_value(flipped_x, 0, WIDTH, 0, 180))
    
    # Set constant angles so that the arm always points centre up.
    # Adjust these constants as needed.
    angle_shoulder = 60   # Shoulder fixed to lift the arm upward
    angle_elbow = 120     # Elbow fixed to achieve an upright configuration

    # Clamp values just in case
    angle_base = max(0, min(180, angle_base))
    angle_shoulder = max(0, min(180, angle_shoulder))
    angle_elbow = max(0, min(180, angle_elbow))
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # Use index finger tip for x coordinate tracking
                index_finger = hand_landmarks.landmark[8]
                x = int(index_finger.x * WIDTH)
                y = int(index_finger.y * HEIGHT)

                # Only x-coordinate drives movement; y is not used now.
                send_angles(x, y)

                # Visualize the point used for tracking
